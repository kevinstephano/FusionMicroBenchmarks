...
def gen_mha_args (seqs, hidden_dim, low, high, step) :
    name_list = []
    input_list = []
    is_bool_list = []
    grad_list = []
    for idx in range(low, high+step, step) :
        ...
        input_list.append([[idx, seqs, hidden_dim], [seqs, 1, 1, idx]])
        is_bool_list.append([False, True])
        grad_list.append([[idx, seqs, hidden_dim]])
    return name_list,input_list,is_bool_list,grad_list

def gen_4d_args (seqs, heads, low, high, step) :
    name_list = []
    input_list = []
    is_bool_list = []
    grad_list = []
    for idx in range(low, high+step, step) :
        name_list.append("_sq" + str(seqs) + "_hd" + str(heads) + "_ln" + str(idx))
        input_list.append([[seqs, heads, idx, idx], [seqs, 1, 1, idx]])
        is_bool_list.append([False, True])
        grad_list.append([[seqs, heads, idx, idx]])
    return name_list,input_list,is_bool_list,grad_list
